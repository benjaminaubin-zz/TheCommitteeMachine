...
class StateEvolution(object):
	...
	def __init__(self,K=2,PW_choice='binary',alpha=1,channel='sign-sign',save_backup_running=False,seed=False,committee_symmetry=False,print_running=True):
		...
		self.borne_inf = -10
		self.epsabs = 1e-7
		if self.K >2 : # To speed up - does not change really the precision!
			self.borne_sup = 3
			self.borne_inf = -3
			self.epsabs = 1e-5 
		...
		self.difference = 1
		if self.K >2 :
			self.precision = 1e-5 # Iteration SE
			self.N_step_SE = 2000
		else : 
			self.precision = 1e-6
			self.N_step_SE = 2000
		...
		## damping
		self.damping_coef = 0.01
		self.damping_coef_slow = 0.01
		## condition to break
	...
	def initialization(self):
		...
		test_spd = False
		while not test_spd:
			coef_q =  np.random.random() /2 
			coef_qhat =  np.random.random() /2 
			
			mode = self.initialization_mode
			if self.K ==1 or self.channel != 'sign-sign':
				mode = 4

			if mode == 1: 
				self.q = 0.999 * np.identity(self.K) 
				self.qhat = 0.999 * np.identity(self.K)

			# diagonal random
			if mode == 2 :
				self.q = coef_q * np.diag(np.random.random((self.K,),))
				self.qhat = coef_qhat * np.diag(np.random.random((self.K,)))
			...
			if mode == 5 :
				try :
					...
					#path = '/Data/BackupObj_SE_'+ self.PW_choice +'_K='+ str(self.K)+'/'
					...
					if np.amax(self.tab_q[:,:,index]) > 0.95 :
						self.q = self.tab_q[:,:,index] - 0.1 * np.identity(self.K)
						self.qhat = self.tab_qhat[:,:,index] - self.tab_qhat[0,0,index]/2 * np.identity(self.K)
						self.damping_coef = self.damping_coef_slow
					# otherwise load backup
					else :
			...
			if self.committee_symmetry :
				...
				for r in range(0,self.K):
					for r_p in range(0,r+1) :
						...
						#print('q[',r,',',r_p,']=',q[r,r_p])
						qhat[r,r_p] = 2 * self.alpha * self.derivative_IZ(r,r_p)
						#print('qhat[',r,',',r_p,']=',qhat[r,r_p])

						# Symmetric matrices X[r_p,r] = X[r,r_p]
						q[r_p,r] = q[r,r_p]
						qhat[r_p,r] = qhat[r,r_p]
						#print('q=',q[r,r_p],'qhat=',qhat[r,r_p])

		end = time.time() 
		#print('Time ellapsed for the step =',end - start)
	...
	def SE_iteration(self):
		...
		#print('committee_symmetry=',self.committee_symmetry,'\n')
	...
	def derivative_IW(self,r,r_p):
		...
		if self.PW_choice == 'binary':
			if self.K == 1 :
				...
				resul = self.derivative_IW_integral_binary_K3(r,r_p)
			else : 
	...
	def integrand_derivative_IW_integral_binary_K2(self,xi1,xi2,r,r_p):
		...
		return resul
	## dIW for K=3
	def derivative_IW_integral_binary_K3(self,r,r_p):
		resul_integral = nquad(self.integrand_derivative_IW_integral_binary_K3, [ [self.borne_inf , self.borne_sup ] , [self.borne_inf , self.borne_sup ]  ,[self.borne_inf , self.borne_sup ] ], args=(r,r_p))[0]
		resul = 1/2 *resul_integral
		return resul
	def integrand_derivative_IW_integral_binary_K3(self,xi1,xi2,xi3,r,r_p):
	...
	def derivative_IZ_y(self,y,r,r_p):
		...
		# K == 3
			...
			#self.step_integration = 0
			#self.time_start = time.time()
			res = nquad(self.integrand_derivative_IZ_y_argument_K3, [ [self.borne_inf , self.borne_sup ] , [self.borne_inf , self.borne_sup ],[self.borne_inf , self.borne_sup ]  ] , args=(y,r,r_p,),opts={'epsabs':self.epsabs})[0]

		#print('y=',y,res)
		
		return res
	def integrand_derivative_IZ_y_argument_K3(self,xi1,xi2,xi3,y,r,r_p):
		xi = np.array([xi1,xi2,xi3])
		gaussian_measure = exp(-1/2* xi.transpose().dot(xi) ) / (2*pi)**(self.K/2) 
		...
		#if self.step_integration % 1000 == 0 :
			#print('step=',self.step_integration,'time=',time.time()-self.time_start)
		#self.step_integration +=1 
	...
	def integrals_fout_gout(self,y,xi,r,r_p):
		...
		if self.channel == 'linear-relu':
			self.step_integral +=1 
			print('step integral',self.step_integral)
			#gout_lr = gout_linear_relu(K=self.K,y=y,omega=omega,V=V,V_inv=V_inv)
			gout_lr = gout_cython_linear_relu(K=self.K,y=y,omega=omega,V_inv=V_inv)
			(fout , gout) = gout_lr.fout_gout()
	...
	def integrals_fout(self,y,xi,r,r_p):
		...
		#print('y=',y,V,V_inv,omega,'fout=',fout)
		return (fout)
...
class FreeEnergy(object):
	def __init__(self,K=2,PW_choice='binary'):
		...
		self.path_BackupObj = 'Data/'
		self.filename_object = self.path_BackupObj + 'SE_FreeEnergy_'+PW_choice+'_K='+str(K)+'.pkl'

		self.load_old_version()
		self.plotFig = True

	def append(self,alpha,Phi,q,qhat,Phi_q1):
		(self.tab_alpha).append(alpha)
		(self.tab_Phi).append(Phi)
		(self.tab_Phi_q1).append(Phi_q1)
		(self.tab_q).append(q)
		(self.tab_qhat).append(qhat)
		save_object(self,self.filename_object)
	def load_old_version(self):
		try : 
			old_object = load_object(self.filename_object)
			self.tab_alpha = old_object.tab_alpha
			self.tab_Phi = old_object.tab_Phi
			self.tab_Phi_q1 = old_object.tab_Phi_q1
			self.tab_q = old_object.tab_q
			self.tab_qhat = old_object.tab_qhat

			#print('Erase old version')
			#save_object(self,self.filename_object)
		except : 
			print('New version')
			#save_object(self,self.filename_object)
	def plot_Phi(self):
		fig, ax1 = plt.subplots(figsize=[8,8])
		tab_alpha = np.array(self.tab_alpha)
		tab_Phi = -np.array(self.tab_Phi)[:,0,0]
		tab_Phi_q1 = -np.array(self.tab_Phi_q1)
		tab_q = np.array(self.tab_q)[:,0,0]
		tab_qhat = np.array(self.tab_qhat)[:,0,0]
		print(tab_q)
		print(tab_qhat)

		tab_l = []
		l, = ax1.plot(tab_alpha,tab_Phi,'-o',color='k',Markersize =3, Linewidth=Linewidth,label=r'Free energy $\Phi$')
		tab_l.append(l)
		l, = ax1.plot(tab_alpha,tab_Phi_q1,'-',color='r',Markersize =5, Linewidth=Linewidth,label=r'Free energy $\Phi_{q=1}$')
		tab_l.append(l)
